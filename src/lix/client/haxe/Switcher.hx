...
class Switcher {
  ...
  static var PLATFORM =
    switch [Sys.systemName(), js.Node.process.arch] {
      case ['Windows', _]: 'windows${isWin64Platform() ? "64" : ""}';
      case ['Mac', _]: 'mac';
  ...
  static public function ensureNeko(logger:Logger):Promise<String> {
    ...
    return
      if (neko.exists())
        ...
        (switch [Sys.systemName(), js.Node.process.arch] {
          case ['Windows', _]: Download.zip('https://github.com/HaxeFoundation/neko/releases/download/v2-3-0/neko-2.3.0-win${isWin64Platform() ? "64" : ""}.zip',
            1, neko, logger);
	...
	static function isWin64Platform() {
		for (environmentVariable in ["PROCESSOR_ARCHITECTURE", "PROCESSOR_ARCHITEW6432"]) {
			...
			if (processorArchitecture != null && processorArchitecture.contains("64"))
				return true;
		}
